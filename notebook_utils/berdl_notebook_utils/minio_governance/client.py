"""
Modern API client for BERDL MinIO Data Governance service

This client provides integration with the BERDL Data Governance API for managing
...
class DataGovernanceClient:
    """
    API client for BERDL MinIO Data Governance service.

    This class provides methods to interact with the BERDL Data Governance API,
    including user workspace management, MinIO credentials, and data sharing operations.
    """
    ...
    def __init__(self, kbase_token: Optional[str] = None):
        ...
        - GOVERNANCE_API_URL: Base URL for the data governance API
        - KBASE_AUTH_TOKEN: KBase authentication token (if kbase_token not provided)
        """
        # Read required environment variables
        self.base_url = str(
            not_falsy(
                os.environ.get("GOVERNANCE_API_URL"),
                "GOVERNANCE_API_URL",
            )
        )
        self.kbase_token = kbase_token or str(not_falsy(os.environ.get("KBASE_AUTH_TOKEN"), "KBASE_AUTH_TOKEN"))
    ...
    def get_group_sql_warehouse_prefix(self, group_name: str) -> GroupSqlWarehousePrefixResponse:
        ...
        return self._get(f"/workspaces/me/groups/{group_name}/sql-warehouse-prefix", GroupSqlWarehousePrefixResponse)
